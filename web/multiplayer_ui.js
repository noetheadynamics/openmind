class MultiplayerUI {
    ...
    updatePlayerList() {
        ...
        listEl.innerHTML = this.playerList.map(p => `
            <div class="mp-player">
                <div class="mp-player-dot" style="background:${this._escapeHtml(p.color)}"></div>
                <div class="mp-player-name">${this._escapeHtml(p.name)}${p.isHost ? ' (Host)' : ''}</div>
                <div class="mp-player-role ${p.role === 'admin' ? 'admin' : ''}">${this._escapeHtml(p.role)}</div>
                ${!p.isHost && this.isHost ? `<button class="mp-player-kick" data-peer="${this._escapeHtml(p.id)}">Kick</button>` : ''}
            </div>
        `).join('');
        listEl.querySelectorAll('.mp-player-kick').forEach(btn => {
            btn.addEventListener('click', () => { this.kickPlayer(btn.dataset.peer); });
        });
    }
