class ChatSystem {
    ...
    renderMessages() {
        ...
        const html = this.messages.map(msg => {
            if (msg.type === 'system') {
                return '<div class="chat-msg chat-system" style="color:' + this._escapeHtml(msg.color || '#eab308') + '">' + this._escapeHtml(msg.text) + '</div>';
            }
            return '<div class="chat-msg">' +
                '<span class="chat-name" style="color:' + this._escapeHtml(msg.color || '#ffffff') + '">' + this._escapeHtml(msg.playerName) + ':</span> ' +
