class WorldEditor {
    ...
    setSelectedType(type) {
        ...
        if (this.renderer) this.renderer.selectedBlockType = type;
    ...
    removeBlock() {
        ...
        const result = this.engine.setBlock(target.x, target.y, target.z, 0);
        if (result !== null) {
            this.renderer.blocks.delete(key);
            this.renderer.interactives.delete(key);
            this.renderer.dirty = true;
            this.breakCount++;
            this.emit('break', { x: target.x, y: target.y, z: target.z });
            return true;
        }
        return false;
    }
