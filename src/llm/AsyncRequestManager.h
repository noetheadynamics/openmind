...
public:
    ...
    void processQueue();
    void setMaxConcurrent(int max) { maxConcurrent.store(max); }
    void setRateLimit(int perMinute) { rateLimitPerMinute.store(perMinute); }
...
private:
    void workerThread();
    bool processRequest(AsyncRequest& request);
    ...
    std::atomic<int> maxConcurrent{4};
    std::atomic<int> rateLimitPerMinute{60};
