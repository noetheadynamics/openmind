...
void OpenAIClient::initialize(const LLMConfig& cfg) {
    config = cfg;
    if (config.baseUrl.empty()) config.baseUrl = "https://api.openai.com/v1";
    if (config.model.empty()) config.model = "gpt-4o";
...
std::string OpenAIClient::buildRequestBody(const std::string& prompt, const std::string& systemInstruction) {
...
std::string OpenAIClient::extractContent(const std::string& body) {
...
int OpenAIClient::extractTokens(const std::string& body, const std::string& field) {
...
LLMResponse OpenAIClient::sendPrompt(const std::string& prompt, const std::string& systemInstruction) {
    LLMResponse resp;
    ...
    reportMetrics("OpenAI", resp.promptTokens + resp.completionTokens, latencyMs);
    return resp;
}
