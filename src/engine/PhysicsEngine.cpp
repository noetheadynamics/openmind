...
void PhysicsEngine::corrodeBlock(VoxelOctree& world, int x, int y, int z, PhysicsData& pd, const MaterialProps& mp, float dt) {
    // Intentionally empty - corrosion logic is handled by processCorrosion()
}
    ...
    for (int x = 0; x < scanRange; x++) {
        for (int y = 0; y < scanRange; y++) {
            for (int z = 0; z < scanRange; z++) {
                ...
                PhysicsData& pd = physicsDataMap[posHash(x, y, z)];
                if (pd.bioStage == BiologicalStage::SEED && pd.growthTimer == 0.0f) {
...
void PhysicsEngine::emitSound(float x, float y, float z, float level, float range, const std::string& type) {
    ...
    if (soundSources.size() < 1024) soundSources.push_back(source);
}
...
void PhysicsEngine::emitLight(float x, float y, float z, float radius, float intensity, const std::string& color) {
    ...
    if (lightSources.size() < 1024) lightSources.push_back(ls);
}
        ...
        for (int y = 0; y < scanRange; y++) {
            for (int z = 0; z < scanRange; z++) {
                ...
                if (world.getBlock(x, y, z, data) && data.type != BlockType::AIR) {
                    snap.voxels.push_back({x, y, z, data});
                }
            }
        }
    }
...
void PhysicsEngine::restoreSnapshot(VoxelOctree& world, const WorldSnapshot& snapshot) {
    ...
    for (const auto& entry : snapshot.voxels) {
        if (entry.x < scanRange && entry.y < scanRange && entry.z < scanRange) {
            world.setBlock(entry.x, entry.y, entry.z, entry.data.type, entry.data.props);
            world.setBlockTemperature(entry.x, entry.y, entry.z, entry.data.currentTemperature);
            world.setBlockState(entry.x, entry.y, entry.z, entry.data.state);
        }
    }
}

bool PhysicsEngine::rewindTime(VoxelOctree& world, int ticks) {
    ...
    if (bestIdx >= 0) {
        restoreSnapshot(world, snapshotBuffer[bestIdx]);
        return true;
    }
    return false;
}
...
void PhysicsEngine::processChainReactions(VoxelOctree& world, int x, int y, int z, float power, int depth) {
    ...
    std::queue<std::tuple<int,int,int,float,int>> explosionQueue;
    explosionQueue.push({x, y, z, power, depth});

    while (!explosionQueue.empty()) {
        auto [cx, cy, cz, curPower, curDepth] = explosionQueue.front();
        explosionQueue.pop();

        if (curDepth >= MAX_CHAIN_DEPTH) continue;
        ...
        for (int dx = -radiusInt; dx <= radiusInt; dx++) {
            for (int dy = -radiusInt; dy <= radiusInt; dy++) {
                for (int dz = -radiusInt; dz <= radiusInt; dz++) {
                    ...
                    float dist = std::sqrt(static_cast<float>(dx*dx + dy*dy + dz*dz));
                    float triggerForce = curPower / (dist * dist + 1.0f);
                    float integrity = nd.props.health.currentHealth / nd.props.health.maxHealth;
                    if (integrity < 0.01f) integrity = 0.01f;

                    if (triggerForce > integrity * 50.0f) {
                        uint64_t key = posHash(nx, ny, nz);
                        if (!explosionProcessed[key]) {
                            explosionProcessed[key] = true;

                            float chainPower = nd.props.chemical.explosivePower;
                            float chainRadius = nd.props.chemical.explosionRadius;
                            if (chainRadius <= 0.0f) chainRadius = chainPower * 0.15f;
                            int chainRadiusInt = static_cast<int>(std::ceil(chainRadius));

                            applyShockwave(world, nx, ny, nz, chainPower, chainRadius);
                            spawnFragments(world, nx, ny, nz, static_cast<int>(chainPower * 0.3f), chainPower);
                            world.setBlock(nx, ny, nz, BlockType::AIR);

                            for (int dx2 = -chainRadiusInt; dx2 <= chainRadiusInt; dx2++) {
                                for (int dy2 = -chainRadiusInt; dy2 <= chainRadiusInt; dy2++) {
                                    for (int dz2 = -chainRadiusInt; dz2 <= chainRadiusInt; dz2++) {
                                        if (dx2 == 0 && dy2 == 0 && dz2 == 0) continue;
                                        float d2 = std::sqrt(static_cast<float>(dx2*dx2 + dy2*dy2 + dz2*dz2));
                                        if (d2 > chainRadius) continue;
                                        float heatDelta = chainPower * 15.0f / (d2 * d2 + 1.0f);
                                        int tx = nx + dx2, ty = ny + dy2, tz = nz + dz2;
                                        VoxelData td;
                                        if (world.getBlock(tx, ty, tz, td) && td.type != BlockType::AIR) {
                                            world.setBlockTemperature(tx, ty, tz, td.currentTemperature + heatDelta);
                                        }
                                    }
                                }
                            }

                            int pcount = static_cast<int>(chainPower * 0.5f);
                            for (int i = 0; i < pcount; i++) {
                                Particle p;
                                p.x = static_cast<float>(nx);
                                p.y = static_cast<float>(ny);
                                p.z = static_cast<float>(nz);
                                std::uniform_real_distribution<float> dir(-1.0f, 1.0f);
                                p.vx = dir(rng) * chainPower * 0.1f;
                                p.vy = std::abs(dir(rng)) * chainPower * 0.1f;
                                p.vz = dir(rng) * chainPower * 0.1f;
                                p.life = 1.0f;
                                p.maxLife = 1.0f;
                                p.size = 0.5f;
                                p.type = "explosion";
                            ...
                            explosionQueue.push({nx, ny, nz, chainPower, curDepth + 1});
                        }
                    }
                }
            }
