...
public:
    ...
    ToolResult executeTool(const std::string& toolName, const std::vector<std::string>& args,
        ...
        if (toolName == "move_to" && args.size() >= 3) {
            return moveTo(ax, ay, az, std::stoi(args[0]), std::stoi(args[1]), std::stoi(args[2]));
        }
        if (toolName == "place_block" && args.size() >= 4) {
            return placeBlock(std::stoi(args[0]), std::stoi(args[1]), std::stoi(args[2]), args[3]);
        }
        if (toolName == "break_block" && args.size() >= 3) {
            return breakBlock(std::stoi(args[0]), std::stoi(args[1]), std::stoi(args[2]));
        }
        if (toolName == "scan_inventory") {
            return scanInventory(agentId);
        }
        if (toolName == "talk_to" && args.size() >= 2) {
            return talkTo(agentId, std::stoi(args[0]), args[1]);
        }
        ...
        if (toolName == "build_structure" && args.size() >= 5) {
            return buildStructure(args[0], std::stoi(args[1]), std::stoi(args[2]),
