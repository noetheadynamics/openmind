...
public:
    ...
    void tick(uint64_t currentTick) {
        ...
        hunger = std::min(100.0f, hunger + config.memoryDecayRate * 10);
        ...
        if (argsPos != std::string::npos) {
            ...
            if (arrStart != std::string::npos && arrEnd != std::string::npos) {
                ...
                if (tool == "move_to" && args.size() >= 3) {
                    action.targetX = std::stoi(args[0]);
                    action.targetY = std::stoi(args[1]);
                    action.targetZ = std::stoi(args[2]);
                }
                    ...
                    action.targetAgentId = std::stoi(args[0]);
                    action.message = args[1];
                }
                else if (tool == "build_structure" && args.size() >= 5) {
                    action.blockType = args[0];
                    action.targetX = std::stoi(args[1]);
                    action.targetY = std::stoi(args[2]);
                    action.targetZ = std::stoi(args[3]);
                }
