...
std::vector<Fragment> PhysicsEngine::getPendingFragments() const { return pendingFragments; }
...
void PhysicsEngine::handleDeformation(PhysicsData& pd, float stress, float elasticLimit) {
    if (stress > elasticLimit && pd.isRigid) {
        ...
        pd.offsetY *= 0.9f;
        pd.offsetZ *= 0.9f;
        if (std::abs(pd.offsetX) < 0.001f && std::abs(pd.offsetY) < 0.001f) {
            pd.isRigid = true;
        }
    }
}
    ...
    for (int x = 0; x < scanRange; x++) {
        for (int z = 0; z < scanRange; z++) {
            for (int y = scanRange - 1; y >= 0; y--) {
                ...
                PhysicsData pd;
                pd.temperature = data.props.thermal.meltingPoint * 0.5f;
...
void PhysicsEngine::handlePhaseChange(VoxelOctree& world, int x, int y, int z, PhysicsData& pd, const MaterialProps& mp) {
    (void)world; (void)x; (void)y; (void)z; (void)pd; (void)mp;
}
...
void PhysicsEngine::expandGas(VoxelOctree& world, int x, int y, int z, const MaterialProps& gasProps) {
    ...
    for (int i = 0; i < 6 && spawned < maxSpawn; i++) {
        ...
        world.setBlock(nx, ny, nz, BlockType::WATER, gasProps);
    ...
    for (int x = 0; x < scanRange; x++) {
        for (int y = 0; y < scanRange; y++) {
            for (int z = 0; z < scanRange; z++) {
                ...
                handlePhaseChange(world, x, y, z, pd, data.props);
            }
        }
    }
}
...
float PhysicsEngine::getBuoyancyForce(float blockDensity, float fluidDensity) const {
    return (fluidDensity - blockDensity) * GRAVITY_EARTH;
}
    ...
    for (int x = 0; x < scanRange; x++) {
        for (int y = 0; y < scanRange; y++) {
            for (int z = 0; z < scanRange; z++) {
                ...
                int waterZ = getWaterHeight(world, x, z);
                if (waterZ > y) {
                    float depth = static_cast<float>(waterZ - y);
                    float pressure = WATER_DENSITY * gravity * depth * 0.001f;
                }
            }
        }
    }
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (totalEnergy > 0) {
            ...
            if (!rxn->byproduct.empty()) {
                ...
                if (bx >= 0 && bx < scanRange && bz >= 0 && bz < scanRange) {
                    world.setBlock(bx, y + 1, bz, BlockType::CUSTOM, byprod);
                    world.setBlockTemperature(bx, y + 1, bz, reactionTemp);
                }
            }
    ...
    for (int x = 0; x < scanRange; x++) {
        for (int y = 0; y < scanRange; y++) {
            for (int z = 0; z < scanRange; z++) {
                ...
                PhysicsData pd;
                pd.health = data.props.health.currentHealth;
                pd.isOnFire = false;
            ...
            for (int z = 0; z < scanRange; z++) {
                for (int y = scanRange - 1; y >= 0; y--) {
                    ...
                    if (data.props.biological.growthRate > 0.0f) {
                        world.setBlock(x, y, z, data.type, data.props);
                    }
                    break;
                }
            }
