...
private:
    std::vector<DialogueLine> parseDialogue(const std::string& json,
        ...
        if (arrStart == std::string::npos) return lines;

        size_t arrEnd = json.find(']', arrStart);
        if (arrEnd == std::string::npos) return lines;

        std::string arr = json.substr(arrStart + 1, arrEnd - arrStart - 1);

        size_t pos = 0;
        while ((pos = arr.find("\"speaker\"", pos)) != std::string::npos) {
            DialogueLine line;

            size_t speakerVal = arr.find('"', pos + 9);
            if (speakerVal != std::string::npos) {
                speakerVal++;
            ...
            pos = (textPos != std::string::npos) ? textPos + 6 : pos + 9;
        }
