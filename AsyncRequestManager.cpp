...
void AsyncRequestManager::processRequest(AsyncRequest& request) {
    if (!canMakeRequest()) {
...
void AsyncRequestManager::setMaxConcurrent(int max) { maxConcurrent = max; }
void AsyncRequestManager::setRateLimit(int perMinute) { rateLimitPerMinute = perMinute; }
